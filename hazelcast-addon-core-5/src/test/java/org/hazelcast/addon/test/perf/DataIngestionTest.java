...
public class DataIngestionTest implements Constants
{
	...
	private void init() throws IOException
	{
		...
		hazelcastInstance.getLoggingService().getLogger(hazelcastInstance.getName());
	}
		...
		public void __run()
		{
			...
			for (int i = threadStartIndex; i <= threadStopIndex; i+=MEMBER_SET_SIZE) {
				...
				for (int k = 0; k < MEMBER_SET_SIZE; k++) {
					imap.put(new EligKey(key, key, (short) keyIndex, 
	...
	class ProfilePutThread extends AbstractThread
	{
		...
		public void __run()
		{
			...
			for (int i = threadStartIndex; i <= threadStopIndex; i++) {
				key = prefix + keyIndex;
				imap.put(new ClientProfileKey(key, key, key), new Blob(data));
				putCount++;
				keyIndex++;
			}
			long stopTime = System.currentTimeMillis();
