...
/**
 * GeodeKryoGenerator generates the KryoSerializer class for a given package that
 * contains the data classes. Please see the usages for details.
 * 
 * @author dpark
 *
 */
public class GeodeKryoGenerator {
	...
	public GeodeKryoGenerator(String packageName, String jarPath, int typeId, String srcDir) {
	...
	public void generateKryoSerializer()
		...
		InputStream is = this.getClass().getClassLoader().getResourceAsStream("GeodeKryoSerializerTemplate.txt");
		...
		writeLine();
		writeLine("To register KryoSerializer, add the following lines in the Geode/GemFire configuration file.");
		writeLine("    <serialization-registration>");
		writeLine("        <serializers>");
		writeLine("             <class-name>" + packageName + ".KryoSerializer" + "</class-name>");
		writeLine("        </serializers>");
		writeLine("    </serialization-registration>");
		writeLine();
	}
	...
	private static void usage() {
		String executableName = System.getProperty(PROPERTY_executableName, GeodeKryoGenerator.class.getName());
		writeLine();
		writeLine("NAME");
