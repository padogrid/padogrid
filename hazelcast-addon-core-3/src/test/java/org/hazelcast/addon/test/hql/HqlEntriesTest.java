...
//	@Test
	public void testEntries() throws HqlException {
		HqlQuery<Map.Entry<String, Order>> hql = HqlQuery.newHqlQueryInstance(hz);
		String query = "select * from nw/orders.entries e order by e.value.shipCountry, e.value.customerId desc";
		...
		do {
			...
			for (Map.Entry<String, Order> entry : col) {
				...
				System.out.println(i++ + ". key=" + key + ", order=" + order.getShipCountry() + ", "
