...
- Fixed the bug that improperly intializes PadoGrid. In the previous release, if an app is run immediately after the rwe initenv.sh is directly sourced in from .bashrc, for example, then the app does not recognize the cluster product and fails with a "not found" error.
