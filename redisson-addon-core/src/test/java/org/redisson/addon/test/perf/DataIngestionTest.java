...
import org.redisson.addon.test.perf.TransactionTest.TestCaseEnum;
 ...
 * mock data into the {@link MapNameEnum#eligibility} and
 * {@link MapNameEnum#profile} maps. It launches multiple threads to
...
public class DataIngestionTest implements Constants
{
	...
	private void runTest(MapNameEnum mapNameEnum, int batchSize, int totalEntryCount, int threadCount, int payloadSize, String prefix, TestCaseEnum testCaseEnum) throws Exception
	{
	...
	abstract class AbstractThread extends Thread
	{
		...
		AbstractThread(int threadNum, MapNameEnum mapNameEnum, int batchSize, int threadStartIndex, int entryCountPerThread, int payloadSize, String prefix)
		{
			this.threadNum = threadNum;
			this.batchSize = batchSize;
			this.threadStartIndex = threadStartIndex;
			this.entryCountPerThread = entryCountPerThread;
			rmap = redisson.getMap(mapNameEnum.name());
	...
	{
		public EligibilityPutThread(int threadNum, MapNameEnum mapNameEnum, int batchSize, int threadStartIndex,
		...
		public void __run()
		{
			...
			for (int i = threadStartIndex; i <= threadStopIndex; i+=MEMBER_SET_SIZE) {
				...
				for (int k = 0; k < MEMBER_SET_SIZE; k++) {
					rmap.put(new EligKey(key, key, (short) keyIndex, 
	...
	class ProfilePutThread extends AbstractThread
	{
		...
		public ProfilePutThread(int threadNum, MapNameEnum mapNameEnum, int batchSize, int threadStartIndex, int entryCountPerThread, int payloadSize, String prefix)
		{
			super(threadNum, mapNameEnum, batchSize, threadStartIndex, entryCountPerThread, payloadSize, prefix);
		}
		...
		public void __run()
		{
			...
			for (int i = threadStartIndex; i <= threadStopIndex; i++) {
				key = prefix + keyIndex;
				rmap.put(new ClientProfileKey(key, key, key), new Blob(data));
				putCount++;
				keyIndex++;
			}
			long stopTime = System.currentTimeMillis();
	...
	class ProfilePutAllThread extends AbstractThread
	{
		...
		public ProfilePutAllThread(int threadNum, MapNameEnum mapNameEnum, int batchSize, int threadStartIndex, int entryCountPerThread, int payloadSize, String prefix)
		{
			super(threadNum, mapNameEnum, batchSize, threadStartIndex, entryCountPerThread, payloadSize, prefix);
		}
