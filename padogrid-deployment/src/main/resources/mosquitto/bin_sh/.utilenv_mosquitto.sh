...
function getVmMosquittoMemberPid
{
   ...
   local pid=`ssh -n $VM_KEY $VM_USER@$__HOST -o LogLevel=error -o stricthostkeychecking=no -o connecttimeout=$SSH_CONNECT_TIMEOUT "ps -eo pid,comm,args | grep mosquitto | grep $__MEMBER_PORT | grep -v grep | awk '{print \\$1}'"`
   echo $pid
}
...
function getMosquittoServerPortPid
{
   ...
   local pid=$(ps -eo pid,comm,args | grep mosquitto | grep $MEMBER_PORT | grep -v grep | awk '{print $1}')
   echo $pid
}
...
function getMosquittoMemberPid
{
   ...
   if [ "$__IS_GUEST_OS_NODE" == "true" ] && [ "$POD" != "local" ] && [ "$REMOTE_SPECIFIED" == "false" ]; then
     __MEMBER_PORT=$__MEMBER_START_PORT
      pid=$(ssh -n $SSH_USER@$NODE_LOCAL -o LogLevel=error -o stricthostkeychecking=no -o connecttimeout=$SSH_CONNECT_TIMEOUT "ps -eo pid,comm,args | grep mosquitto |grep $__MEMBER_PORT | grep -v grep")
      pid=$(echo $pid | awk '{print $1}')
   else
      if [ "$POD" != "local" ]; then
         __MEMBER_PORT=$__MEMBER_START_PORT
      fi
      pid=$(ps -eo pid,comm,args | grep mosquitto | grep $__MEMBER_PORT | grep -v grep | awk '{print $1}')
   fi
   echo $pid
}
