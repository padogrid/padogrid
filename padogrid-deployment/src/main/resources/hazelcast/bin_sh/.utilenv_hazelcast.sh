...
function getMcPid
{
   ...
   if [ "$__RWE" == "" ]; then
      ...
      if [[ "$OS_NAME" == "CYGWIN"* ]]; then
         ...
         local mcs="$(ps -wweo pid,comm,args | grep java | grep padogrid.hazelcast.mc.name=$__MC | grep padogrid.workspace=$__WORKSPACE | grep padogrid.rwe=$__RWE | grep -v grep | awk '{print $1}')"
      fi
   else
      # Use eval to handle commands with spaces
      if [[ "$OS_NAME" == "CYGWIN"* ]]; then
         local mcs="$(WMIC path win32_process get Caption,Processid,Commandline | grep java | grep padogrid.hazelcast.mc.name=$__MC | grep padogrid.workspace=$__WORKSPACE | grep padogrid.rwe=$__RWE | grep -v grep | awk '{print $(NF-1)}')"
      else
         local mcs="$(ps -wweo pid,comm,args | grep java | grep padogrid.hazelcast.mc.name=$__MC | grep padogrid.workspace=$__WORKSPACE | grep padogrid.rwe=$__RWE | grep -v grep | awk '{print $1}')"
      fi
   fi
