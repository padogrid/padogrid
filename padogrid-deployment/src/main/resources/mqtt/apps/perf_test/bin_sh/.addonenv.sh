...
if [ ! -d "$APP_LOG_DIR/paho" ]; then
   mkdir -p "$APP_LOG_DIR/paho"
fi

# k8s pod
if [ "$NAMESPACE" != "" ] && [ "$MQTT_SERVICE" != "" ]; then
   K8S_PROPERTIES="-Dk8s.mqtt.service=$MQTT_SERVICE -Dk8s.namespace=$NAMESPACE"
   MQTT_CLIENT_CONFIG_FILE=$APP_ETC_DIR/mqttv5-client-k8s.yaml
else
   MQTT_CLIENT_CONFIG_FILE=$APP_ETC_DIR/mqttv5-client.yaml
fi
LOG_CONFIG_FILE=$APP_ETC_DIR/log4j2.properties
export LOG_DIR=$APP_DIR/log

if [[ ${OS_NAME} == CYGWIN* ]]; then
   MQTT_CLIENT_CONFIG_FILE="$(cygpath -wp "$MQTT_CLIENT_CONFIG_FILE")"
