...
`HaMqttClient` maintains the same level of QoS as the underlying MQTT client API (Paho). The only difference is that there are multiple MQTT client instances under the hood, each maintaining the same QoS for a given topic. For publishing topics, `HaMqttClient` publishes to only one of the brokers. For subscribing topics, `HaMqttClient` subscribes to all of the brokers. Since only one of the brokers is used to publish topics, `HaMqttClient` will receive messages from that broker only despite it is subscribed to all of the brokers.
...
The simplest way is to type-cast `IMqttClient` as follows.
