...
tar -C ~/Padogrid/products/ -xzf padogrid_1.0.3-SNAPSHOT.tar.gz
cd ~/Padogrid/products
tree -L 1 padogrid_1.0.3-SNAPSHOT
```

**Output:**

```bash
padogrid_1.0.3-SNAPSHOT
├── LICENSE
├── NOTICE
├── README.md
├── RELEASE_NOTES.md
...
```bash
~/Padogrid/products/padogrid_1.0.3-SNAPSHOT/bin_sh/create_rwe
```
...
✏️  PadoGrid Docker containers follow the same version conventions as the build except for the SNAPSHOT versions which also include a build number starting from 1. For example, the `padogrid/paadogrid:1.0.3-SNAPSHOT-2` image has the build number 2. The SNAPSHOT versions are for testing only and subject to removal without notice.
